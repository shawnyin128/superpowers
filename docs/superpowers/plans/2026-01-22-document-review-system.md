...
> **For agentic workers:** REQUIRED: Use sp-harness:subagent-driven-development (if subagents available) or sp-harness:executing-plans to implement this plan.
...
```markdown
> **For agentic workers:** REQUIRED: Use sp-harness:subagent-driven-development (if subagents available) or sp-harness:executing-plans to implement this plan. Tasks and steps use checkbox (`- [ ]`) syntax for tracking.
```
